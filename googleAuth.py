...
def index():
    ...
    form_url = None
    error_message = None
    form_preview_data = None  # For live preview
    prompt_history = session.get('prompt_history', [])
    ...
    if request.method == "POST":
        ...
        if not prompt:
            ...
            session['prompt_history'] = prompt_history[-5:] # Keep only last 5
            try:
                 # For preview

                # Generate JSON using Gemini
                gemini_response = geminiQuery("""Generate JSON for creating questions in a Google Form quiz based on """+prompt+""" For each question, specify the correct answer. The JSON should follow this format:
{
  "requests": [
    {
    "updateSettings": {
...
Return *only* the JSON object. Do not include any backticks (```), code fences, or explanatory text.                           
                ...
                if form_service:
                    ...
                    question_setting = form_service.forms().batchUpdate(formId=form_id, body=data).execute()
...
if __name__ == "__main__":
    app.run(debug=False, use_reloader=False, port=8000, host='0.0.0.0', ssl_context=('certificate.crt', 'private.key'))
